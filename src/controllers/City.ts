...
export class City {
    async city(req: Request, res: Response) {
        ...
        var url = `https://api.hgbrasil.com/weather?woeid=455867format=json-cors&key=917e1d9a` 
        if(token){
            url = `https://api.hgbrasil.com/weather?woeid=455867format=json-cors&key=${token}`
        }
